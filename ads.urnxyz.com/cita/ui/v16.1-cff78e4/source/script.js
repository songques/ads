...
appRun.$inject = ["$q", "$compile", "initService", "messagesService", "$translate", "sessionData", "$window", "$filter"];
ErrorController.$inject = ["$scope", "$location", "errorService", "initService", "tanService", "confirmService"];
HelpController.$inject = ["$scope", "$resource", "$location", "$translate", "initService"];
MasterController.$inject = ["$scope", "$window", "$translate", "sessionData", "productService", "$location", "initService"];
confirmService.$inject = ["$q", "$resource", "$location", "$window"];
errorService.$inject = ["$routeParams"];
initService.$inject = ["$resource", "$location", "$q", "productService", "msisdnService", "tanService", "confirmService"];
messagesService.$inject = ["$q", "$resource", "initService"];
msisdnService.$inject = ["$q", "$resource", "$location"];
productService.$inject = ["$q"];
tanService.$inject = ["$q", "$resource", "$location", "$window", "productService", "confirmService"];
ConfirmController.$inject = ["$scope", "$timeout", "sessionData", "initService", "tanService", "confirmService", "productService", "$filter"];
focusMe.$inject = ["$timeout", "$parse"];
MsisdnController.$inject = ["$scope", "$timeout", "sessionData", "msisdnService", "tanService", "confirmService"];
PriceController.$inject = ["$scope", "$attrs", "$filter", "$q", "sessionData", "productService"];
TanController.$inject = ["$scope", "$timeout", "sessionData", "msisdnService", "tanService", "confirmService"];angular.module('citaApp', [
    'ngRoute',
...
angular.module('citaApp.services', ['ngResource'])
    ...
    .service('sessionData', ["$q", function ($q) {
        return {sessionId: null, deferred: $q.defer()};
    }])

    .factory('missingTranslationHandler', ["$log", "$translate", function missingTranslationHandlerFactory($log, $translate) {
        return function (translationId) {
            return null;
        };
    }])

    .factory('$exceptionHandler', ["$log", "$injector", "errorService", function ($log, $injector, errorService) {
        var $location = null;
...
+function ($) {
  ...
  Checkbox.prototype.init = function (element, options) {
    ...
    $checkbox.parents(formset).on('click.tc.checkbox', function (e) {
      if (!$(e.target).hasClass('form-checkbox hidden')) {
        $e.trigger('click');
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data) $this.data('tc.checkbox', (data = new Checkbox(this, options)))
      if (typeof option == 'string') data[option]()
    })
  }
...
//      if (disabled) classes.push('disabled')
  ...
  SelectBox.prototype.textInputChangeHandler = function (e) {
    ...
    var desc = 'li:not([aria-disabled="true"])'
    var $itemsEnabled = $this.find(desc)
    if (!$itemsEnabled.length) return
  ...
  ToTop.prototype.generateButton = function (type, replace) {
    ...
    if (replace === true) {
      ...
      finalButton = '<div class="totop">' +
                    ...
                    '<div class="row">' +
                    '<div id="button-wrap" class="col-12">' + button +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    '</div>'
